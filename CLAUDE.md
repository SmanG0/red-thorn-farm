...
**Local development:** Open `index.html` directly in a browser, or use `npx serve .`. Each HTML page hardcodes the production Convex URL as a `const CONVEX_URL` near the top of its inline `<script>` — swap that value to the local dev URL printed by `npx convex dev` when testing locally.

**Deployment:** Static files auto-deploy via Vercel on push. No server — Vercel serves the files as a static site.
...
- `enhancements.js` — progressive enhancement layer (scroll-reveal via IntersectionObserver, scroll progress bar). Safe to remove per-page.
...
Two pages exist that are **not** linked in the main nav: `story.html` (`id: 'story'`) and `visit.html` (`id: 'visit'`). They follow the same RTPAGE pattern but won't show as active in the nav.
...
All backend logic lives in `convex/`. `convex/_generated/` is auto-generated by Convex — do not edit by hand.

**Note:** `convex.json` contains stale fields — the `project` slug (`pilo`) and `prodUrl` are old artifacts. The live production URL is the one hardcoded in the frontend HTML (`https://prestigious-goldfish-965.convex.cloud`).
...
4. `orders:submitOrder` is idempotent — same `submissionId` returns the existing order; it also soft-warns (but still allows) if the same phone has a pending order for the same `deliveryWeek` (always next Friday). Order numbers follow `RT-YYYYMMDD-###` (daily sequence).

**Important:** The product list in `order.html` is hardcoded HTML. The `products` table is used only to overlay "out of stock" labels — the page fetches `products:listAll` on load and matches by `data-product` attribute. Product names in the DB (seeded by `products:seedProducts`) must match those `data-product` values exactly, or the stock-status overlay won't find the card.
